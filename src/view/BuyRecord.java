...
public class BuyRecord extends JFrame {
	...
	public BuyRecord(User user) {
		...
		label.setText("【用户】" + user.getName()+"购买的商品");
		label.setFont(new Font("宋体", Font.PLAIN, 22));
		label.setBounds(180, 10, 360, 34);
		...
		JLabel label_6 = new JLabel("\u4E00\u5171\u8D2D\u4E70");
		...
		int m=user.getId();
		double[] aaa=productsDao1.getProductsNumber(m);
		...
		JLabel lblNewLabel_1 = new JLabel("\u82B1    \u8D39");
	...
	private void setTable(Products products) {
		...
		int a=user.getId();
		List<Products> productsList = productsDao.getProductsListC(a,0);
